...
with st.sidebar:
    st.header("칸의 명")
    mode = st.radio("모드", ["🎲 자유 순행", "⚔️ 순행 임무 (베타)"], horizontal=True, label_visibility="collapsed")
    game_mode = "임무" in mode
    scen = None
    if game_mode and SCENARIOS:
        st.caption("⚔️ **순행 임무 (베타)** — 임무·별점 기능은 시험 중입니다.")
        scen_choice = st.selectbox("임무 선택", [s.get("title", "") for s in SCENARIOS])
        scen = SCEN_BY_TITLE.get(scen_choice)

    if scen:
        # ── 게임 모드(순행 임무): 도시·주체·우선순위·계절을 시나리오로 고정(읽기전용 표시) ──
        sel = [c for c in scen.get("cities", []) if c in BY]
        _st = scen.get("start")
        start = _st if _st in sel else (sel[0] if sel else None)
        ...
        reps, maxiter, show_ghosts = 2, 100, False
        ss["force_mode"] = "draw"   # 임무는 순행 그리기 모드로 시작
        cond = ("지형 ON · %s · %s · %s" % (omode, opriority, oseason)) if terrain_on else "지형 OFF (단순 거리)"
        st.caption("**임무 고정** — 아래 조건으로 순행하세요.")
        st.markdown("- **조건**: %s\n- **출발/귀환**: %s\n- **도시(%d)**: %s"
                    % (cond, start or "-", len(sel), ", ".join(sel)))
    else:
        if game_mode:
            st.warning("등록된 순행 임무가 없습니다. scenarios.json에 추가하세요.")
        # ── 샌드박스(자유 순행): 기존 동작 그대로 ──
...
scenario_meta = {"id": scen.get("id"), "title": scen.get("title"),
                 ...
                 "lore": scen.get("lore", "")} if scen else None
...
html = build_map(CITIES, GHOSTS, sel, answer, start=start, show_ghosts=show_ghosts,
                 ...
                 metrics=metrics3, orbis=orbis, scenario=scenario_meta)
