...
## 25. 버그픽스: "보급선 완성해도 안 끝남" → 보급 링/effReach 불일치 (2026-06-22)

**증상(사용자)**: 역참 미니게임에서 보급로를 완성해도 끝나지 않음. **재현 실패**(자동 탐욕 플레이는 28/28 완성·grade/afterLeg/finish 정상·콘솔0)
→ 수동 플레이 특유의 함정으로 진단. **원인**: 보급 링이 `reachOf(hd)`(기본 사거리, 큰 원)로 그려지는데 실제 도달은 `effReach`(사막서 ÷1.55).
측정: 사막 구간서 **링 안에 있지만 안 닿는 '함정' 노드 2개**(평지 0). 사람은 큰 원 안 노드·도착을 클릭→안 이어짐→"안 끝난다"고 느낌.
- **수정**: ① 보급 링 = **effReach 방향별 샘플 블롭**(30방향, 사막 방향 짧게) — 링 안=실제 닿는 곳(재측정 함정 **2→0**). ② 도착(B0)까지
  닿으면 **B에 금색 펄스**("이으면 완성") + 하단 "도착까지 보급 연결됨 ✓". ③ 사거리 밖 노드 클릭 시 **빨강 깜빡**+select음(왜 안 되는지 피드백).
- 검증: 블롭 함정 0·완성 버튼 활성→grade(rsRetry/rsNext)·B0 펄스 1개·함정 클릭 빨강 깜빡·풀 플레이·콘솔0. 스크린샷(연결됨 ✓·펄스).
  ※진짜 hang은 없었음(흐름 정상) — 도달성 가시화 불일치를 해소한 것. 사용자 재확인 권장.

## 26. 오프닝 내러티브 = 전면 낭독 뷰 (2026-06-25)

사용자가 `scenarios.json` 오프닝 beats를 칼비노 결 장문 산문(베네치아=최초의 도시)으로 채움 → 하단 카드(opCard)엔 비좁아
**중앙 전면 낭독 뷰**로 전환(결정: 전면 낭독 뷰 + 삽화칸 유지).
- HTML `#opRead`(opening 안): 삽화칸·장태그·큰 제목·본문·다음 버튼. CSS: 지도를 어둡게 깔고(라디얼 dim 배경) 중앙 컬럼
  (min 690px, max-h 90% 스크롤), 본문 serif 15.5px·line 1.95·따뜻한 양피지색, 제목 27px. z-8(opCard 위).
- JS: `showRead(b)`(beat 내러티브를 낭독 뷰에, opCard opacity 0)·`hideRead()`. `beat()` 리팩터 — 내러티브=낭독 뷰,
  next_node beat은 다음 클릭 시 `hideRead()`로 닫고 **하단 opCard에 작은 클릭 힌트**("X를 클릭하라")+노드 활성화(지도 밝게).
  순수 내러티브는 닫지 않고 다음 beat가 낭독 뷰 갱신(깜빡임 방지). 삽화칸=`b.img` 있으면 표시·없으면 자리. opCard는 타이틀
  모드선택·클릭 힌트용으로 존치.
- 검증(preview): beat0 낭독 렌더(삽화칸·제노바—옥중·적히지 않은 도시·장문 5줄·스크롤) 스크린샷, 실제 흐름 스토리→제노바
  클릭→beat0 낭독→다음→하단 힌트 "베네치아 클릭"→베네치아 클릭→beat1 "길을 떠나던 날", 콘솔0.
- ※`scenarios.json`(사용자 편집)·`map_view.py` 미커밋. 도착/보고 연출(#cityscene)은 이미 중앙이라 그대로.
